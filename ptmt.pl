...
sub PTMT{ 
	my @file=@_;
	foreach (@file){
		my $file1="/home/zhouxy/CCDB/NUMT/blast_result/".$_."_blastn_ptmt.txt";
		my $file2="/home/zhouxy/CCDB/gff/".$_.".gff";
		my $file3="/home/pub/data/lab/zhouxy/PTMT/".$_."_filter_ptmt.txt";
		my $file4="/home/zhouxy/CCDB/NUMT/blast_result/".$_."_reblastn_ptmt.txt";
		my $file5="/home/pub/data/lab/zhouxy/PTMT/".$_."_statistics_ptmt.csv";
		my $file6="/home/zhouxy/CCDB/fna/repeat/".$_.".fna.out";
		my $file7="/home/pub/data/lab/zhouxy/PTMT/".$_."_judge_ptmt.txt";
		my $file8="/home/pub/data/lab/zhouxy/PTMT/".$_."_test_ptmt.txt";
		my $file9="/home/pub/data/lab/zhouxy/PTMT/".$_."_3filter_ptmt.txt";
		my $file10="/home/pub/data/lab/zhouxy/PTMT/".$_."_MTPT.txt";
		my $file11="/home/pub/data/lab/zhouxy/PTMT/".$_."_PTMT.txt";
		my $file12="/home/pub/data/lab/zhouxy/PTMT/".$_."_2judge_ptmt.txt";
		#grep seq
					...
					for my $i( 0..($#{$gff{$gid}}) ){
						...
						if (($query[1]>=$gstart{$gff})&&($query[1]<$gend{$gff})){
							if (($query[1]<=$gstart{$gff})&&($query[2]>=$gend{$gff})&&(grep /$label{$gff}/,@feature)){
								print OUT "\n$gid\t$label{$gff}(complete)(pt)\t$gstart{$gff}\t$gend{$gff}\t@rest";
							}else {
								print OUT "\n$gid\t$label{$gff}(pt)\t$gstart{$gff}\t$gend{$gff}\t@rest";
							}
				...
				if ($numpse==1){
					foreach $parameter(@judge){
						if ($parameter=~/pseudogene/){
							if ($parameter=~/pt/){
								print JUDGE "pt_mt_pseudogene\t";
								push @count,"pt_mt_pseudogene";
							}else{
								print JUDGE "mt_pt_pseudogene\t";
								push @count,"mt_pt_pseudogene";
							}
						}
					}	
				}elsif($numpse!=1){
					foreach $parameter(@judge){
						...
						if (grep {$_ eq $tmp} @feature){
							if ($parameter=~/complete/){
								if ($parameter=~/(pt)/){
									print JUDGE "pt_mting\t";
									push @count,"pt_mting";
								}else{
									print JUDGE "mt_pting\t";
									push @count,"mt_pting";
								}
							}else{
								if ($parameter=~/(pt)/){
